        ...
        <>
            <div className="navitem">
                {navitems.children ? (<>
                    ...
                    <a target={navitems.target}><span className={`p-4 hover:border-b-2 mr-4 hover:border-b-amber-600 hover:text-amber-600 cursor-pointer ${router.asPath == navitems.path ? 'active' : ''} ${navitems.type == 'cta' ? 'bg-cyan-900 text-white rounded-md' : ''}`}>
                    {navitems.name}
                </span></a></Link></>)}
            </div>
            <style jsx>{`

...
const technologyList = [
  {
    ...
    description:
      "We do <b><u>painless root canal treatment</u></b> with equipment that fasten up the treatment and provide world class results by using one of the best Apexlocator used in modern dentistry by J.Morita and Endo-motor (Motorised Machine based RCT).",
    imageUrls: [
...
export default function TechnologyPage() {
  const backgroundImage = "/assets/periodical-dental-checkup.webp";
  return (
    <>
      <div
        className="bg-center bg-no-repeat bg-cover top-40 backdrop-blur-lg"
        style={{
      ...
      >
        <div className="block p-8">
          <div className="m-auto mt-44 border-4 border-zinc-600 p-4 backdrop-blur-lg bg-white/50 md:w-8/12 top-3.5">
            <div className="m-auto text-center">
              <h1 className="font-bold text-black uppercase md:text-3xl sm:text-xl">Dental Technology we use</h1>
              <p className="m-4 text-left text-black italic md:text-xl sm:text-md">
      ...
      <div className="mt-10 text-center">
        <h2 className="text-xl font-bold text-black">Modern Dental Equipment</h2>
        <div className="grid gap-4 p-4 text-black sm:grid-cols-1 md:m-10 md:grid-cols-2 lg:grid-cols-3 min-h-fit backdrop-blur-sm">
          {technologyList.map((tech, index) => (
            <EquipmentDetail key={`technology-${index}`} title={tech.title} description={tech.description} imgList={tech.imageUrls} />
          ))}
        </div>
      </div>
    </>

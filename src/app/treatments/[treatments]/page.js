...
export async function generateMetadata({ params }) {
  const slug = params.treatments;
  const found = TreatmentList.find((it) => it.slug === slug) || null;
  const t = found?.treatment ?? found;
  const title = t?.name ? `${t.name} | Treatments | Hapliv Dental Clinic` : 'Treatment | Hapliv Dental Clinic';
  return {
    title,
    description: t?.overview?.slice(0, 155),
    keywords: t?.keywords,
    openGraph: {
      type: 'article',
      title,
      description: t?.overview,
      images: t?.heroImage ? [t.heroImage] : undefined,
    },
    twitter: {
            ...
            {items.map((item, idx) => {
              ...
              return (
                <div key={idx} className="p-6 text-center transition-shadow bg-white shadow-md rounded-xl hover:shadow-lg">
                  <div className="flex items-center justify-center mx-auto mb-3 bg-purple-100 rounded-full h-14 w-14">
                    <Icon className="text-purple-700 h-7 w-7" />
                  </div>
            ...
            .map((g) => (
              <div key={g.key} className="p-6 border rounded-xl">
                <div className="flex items-center gap-3 mb-3">
                  <div className="grid bg-purple-100 rounded-full h-9 w-9 place-items-center">
  ...
  return (
    <section className="px-4 py-16 text-white bg-gradient-to-r from-purple-700 via-purple-600 to-purple-800">
      <div className="container max-w-6xl mx-auto text-center">
        ...
        <p className="max-w-2xl mx-auto mb-8 text-lg text-white/75 opacity-90">
